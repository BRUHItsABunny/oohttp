...
// NOTE(oohttp): HTTP/3 is not supported by this fork. The h3 test mode is
// declared so that the (upstream) call sites keep compiling, but it is never
// exercised: run() skips it and newClientServerTest rejects it.
...
func run[T TBRun[T]](t T, f func(t T, mode testMode), opts ...any) {
	...
	for _, mode := range modes {
		// HTTP/3 is not supported by this fork.
		if mode == http3Mode {
			continue
		}
