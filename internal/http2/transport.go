...
import (
	...
	"github.com/BRUHItsABunny/oohttp/internal"
	"github.com/BRUHItsABunny/oohttp/internal/httpcommon"
	"io"
	"io/fs"
	"log"
	"math"
...
// customSetting returns the value the caller configured for the given SETTINGS
// parameter, if custom initial settings are in use.
// It is a github.com/BRUHItsABunny/oohttp extension.
func customSetting(t1 TransportConfig, id SettingID) (uint32, bool) {
	if t1 == nil || !t1.HasCustomInitialSettings() {
		return 0, false
	}
	params := t1.SettingsFrameParameters()
	if int(id) < 1 || int(id) > len(params) {
		return 0, false
	}
	v := params[id-1]
...
func (t *Transport) newClientConn(c net.Conn, singleUse bool, internalStateHook func()) (*ClientConn, error) {
	...
	// oohttp: when the caller supplies custom SETTINGS we must decode with the
	// table size we actually advertise, or the peer's HPACK stream will look
	// like a COMPRESSION_ERROR to us.
	if v, ok := customSetting(t.t1, SettingHeaderTableSize); ok {
	...
	// oohttp: the SETTINGS frame is one of the strongest HTTP/2 fingerprints,
	// so callers may replace it wholesale with the exact parameters (and order)
	// of the client they are impersonating.
	var initialSettings []Setting
	if t.t1 != nil && t.t1.HasCustomInitialSettings() {
		for i, value := range t.t1.SettingsFrameParameters() {
			if value < 0 || value > math.MaxUint32 {
				// Skip because the value is invalid, or the caller asked us
				// not to send this parameter at all.
				continue
			}
			initialSettings = append(initialSettings, Setting{ID: SettingID(i + 1), Val: uint32(value)})
		}
	} else {
	...
	// oohttp: likewise for the initial connection-level WINDOW_UPDATE.
	connFlow := uint32(conf.MaxReceiveBufferPerConnection)
	if t.t1 != nil && t.t1.HasCustomWindowUpdate() {
		connFlow = t.t1.WindowUpdateIncrement()
	}
	...
	// oohttp: browsers open a handful of "phantom" streams with PRIORITY
	// frames before their first request; replay them here.
	if t.t1 != nil {
		if pfs := t.t1.PriorityFrameSettings(); pfs != nil {
			if pfs.HeaderFrame != nil {
				cc.fr.HeaderPriority = &PriorityParam{
					StreamDep: pfs.HeaderFrame.StreamDep,
					Exclusive: pfs.HeaderFrame.Exclusive,
					Weight:    pfs.HeaderFrame.Weight,
				}
			}
			// The slice index is the stream ID of the PRIORITY frame, so a
			// nil entry simply leaves that ID unused. Stream IDs consumed
			// here must not be reused by real requests, and client-initiated
			// streams are always odd.
			for streamID, priority := range pfs.PriorityFrames {
				if priority == nil {
					continue
				}
				cc.fr.WritePriority(uint32(streamID), PriorityParam{
					StreamDep: priority.StreamDep,
					Exclusive: priority.Exclusive,
					Weight:    priority.Weight,
				})
				if next := uint32(streamID) + 2; next > cc.nextStreamID {
					cc.nextStreamID = next | 1
				}
			}
		}
	}
...
func (rl *clientConnReadLoop) handleResponse(cs *clientStream, f *MetaHeadersFrame) (*ClientResponse, error) {
	...
	trackHeaderOrder := cs.cc.t.t1 != nil && cs.cc.t.t1.TrackResponseHeaderOrder()
	var headerOrder []string
	var seenHeaders map[string]bool
	if trackHeaderOrder {
		seenHeaders = make(map[string]bool, len(regularFields))
	}
	res := &cs.staticResp
	...
	for _, hf := range regularFields {
		...
		if key == "Trailer" {
			...
			// oohttp: record the order in which the server sent its headers,
			// which is itself a server fingerprint.
			if trackHeaderOrder && !seenHeaders[key] {
				seenHeaders[key] = true
				headerOrder = append(headerOrder, hf.Name)
			}
			vv := header[key]
	...
	if trackHeaderOrder && len(headerOrder) > 0 {
		header[httpcommon.HeaderOrderKey] = headerOrder
	}
