...
func testTransportNewClientConnRoundTrip(t *testing.T, mode testMode) {
	...
	for _, host := range []string{"example.tld", "go.dev"} {
		req, _ := NewRequest("GET", fmt.Sprintf("%v://%v/", scheme, host), nil)
	...
	req, _ := NewRequest("GET", scheme+"://example.tld/", nil)
	resp, err := cc.RoundTrip(req)
	if err == nil {
		resp.Body.Close()
...
func newClientConnTest(t testing.TB, mode testMode, h HandlerFunc, opts ...any) (*clientServerTest, *ClientConn) {
	if h == nil {
		h = func(w ResponseWriter, req *Request) {}
	}
...
func testClientConnReserveAll(t *testing.T, mode testMode) {
	cst, cc := newClientConnTest(t, mode, nil, optFakeNet, func(s *Server) {
		s.HTTP2 = &HTTP2Config{
...
func testClientConnReserveParallel(t *testing.T, mode testMode) {
	_, cc := newClientConnTest(t, mode, nil, optFakeNet, func(s *Server) {
		s.HTTP2 = &HTTP2Config{
			MaxConcurrentStreams: 3,
		}
	})
...
func TestClientConnReserveAndConsume(t *testing.T) {
	for _, test := range []struct {
		name     string
		consume  func(t *testing.T, cc *ClientConn, mode testMode)
		handler  func(w ResponseWriter, req *Request, donec chan struct{})
		h1Closed bool
	}{{
		// Explicit release.
		name: "release",
		...
		consume: func(t *testing.T, cc *ClientConn, mode testMode) {
			req, _ := NewRequest("GET", mode.Scheme()+"://example.tld/", nil)
		...
		consume: func(t *testing.T, cc *ClientConn, mode testMode) {
			...
			go func() {
				req, _ := NewRequestWithContext(ctx, "GET", mode.Scheme()+"://example.tld/", nil)
				_, err := cc.RoundTrip(req)
				if err == nil {
		...
		t.Run(test.name, func(t *testing.T) {
			runSynctest(t, func(t *testing.T, mode testMode) {
				...
				stateHookCalls := 0
				cc.SetStateHook(func(cc *ClientConn) {
					stateHookCalls++
				})
				synctest.Wait()
				...
				if mode == http1Mode || mode == https1Mode {
					time.Sleep(MaxPostCloseReadTime)
				}
				synctest.Wait()
...
func testClientConnRoundTripBlocks(t *testing.T, mode testMode) {
	...
	_, cc := newClientConnTest(t, mode, handler, optFakeNet, func(s *Server) {
		s.HTTP2 = &HTTP2Config{
			MaxConcurrentStreams: 3,
		}
	})
