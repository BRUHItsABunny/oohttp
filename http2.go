...
func (s *Server) configureHTTP2() {
	...
	if s.TLSNextProto == nil {
		s.TLSNextProto = make(map[string]func(*Server, TLSConn, Handler))
	}
	...
	s.TLSNextProto["h2"] = func(hs *Server, c TLSConn, h Handler) {
		c.Close()
	}
...
func (t *Transport) configureHTTP2(protocols Protocols) {
	...
	t.registerProtocol("https", http2RoundTripper{t, t2, true})
	if t.TLSNextProto == nil {
		t.TLSNextProto = make(map[string]func(authority string, c TLSConn) RoundTripper)
	}
	// Historically, the presence of a TLSNextProto["h2"] key has been the signal to
	// enable/disable HTTP/2 support. Set a value in the map, but we'll never use it.
	t.TLSNextProto["h2"] = func(authority string, c TLSConn) RoundTripper {
...
func http2RoundTrip(t *Transport, req *Request, rt func(*http2.ClientRequest) (*http2.ClientResponse, error)) (*Response, error) {
	resp := &Response{}
	...
	resp.TLS = cresp.TLS
	resp.Request = req
	// oohttp: unlike the stdlib we also decode responses the caller did not
	// explicitly ask us to decode. Clients that spoof a browser's TLS/HTTP2
	// fingerprint send their own Accept-Encoding (br, zstd, ...) and still
	// expect a plaintext body.
	t.decompressResponse(req, resp)
	return resp, nil
}
...
func (t *Transport) http2AddConn(scheme, authority string, nc net.Conn) (RoundTripper, error) {
	...
	return http2RoundTripper{t, t.h2Transport, false}, nil
}
...
func (t *Transport) http2NewClientConn(nc net.Conn, internalStateHook func()) (genericClientConn, error) {
	...
	return http2ClientConn{t, cc}, nil
}
...
func (rt http2RoundTripper) RoundTrip(req *Request) (*Response, error) {
	resp, err := http2RoundTrip(rt.t1, req, rt.t.RoundTrip)
	if err != nil {
...
func (cc http2ClientConn) RoundTrip(req *Request) (*Response, error) {
	return http2RoundTrip(cc.t1, req, cc.NetHTTPClientConn.RoundTrip)
}
...
// The methods below expose this fork's HTTP/2 fingerprinting knobs
// (see Transport) to the net/http/internal/http2 implementation.

func (t transportConfig) HasCustomInitialSettings() bool {
	return t.t.HasCustomInitialSettings
}

func (t transportConfig) SettingsFrameParameters() []int64 {
	return t.t.HTTP2SettingsFrameParameters
}

func (t transportConfig) HasCustomWindowUpdate() bool {
	return t.t.HasCustomWindowUpdate
}

func (t transportConfig) WindowUpdateIncrement() uint32 {
	return t.t.WindowUpdateIncrement
}

func (t transportConfig) PriorityFrameSettings() *http2.ClientPriorityFrameSettings {
	s := t.t.HTTP2PriorityFrameSettings
	if s == nil {
		return nil
	}
	out := &http2.ClientPriorityFrameSettings{}
	if s.HeaderFrame != nil {
		out.HeaderFrame = &http2.ClientPriority{
			StreamDep: s.HeaderFrame.StreamDep,
			Exclusive: s.HeaderFrame.Exclusive,
			Weight:    s.HeaderFrame.Weight,
		}
	}
	for _, p := range s.PriorityFrames {
		if p == nil {
			out.PriorityFrames = append(out.PriorityFrames, nil)
			continue
		}
		out.PriorityFrames = append(out.PriorityFrames, &http2.ClientPriority{
...
func (t transportConfig) TrackResponseHeaderOrder() bool {
	return t.t.TrackResponseHeaderOrder
}

// transportFromH1Transport provides a way for HTTP/2 tests to extract
// the http2.Transport from an http.Transport.
//
//go:linkname transportFromH1Transport github.com/BRUHItsABunny/oohttp/internal/http2_test.transportFromH1Transport
