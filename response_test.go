...
var respTests = []respTest{
	...
	// Unchunked response with Content-Length.
	{
		...
		Response{
			...
			Header: Header{
				"Connection":     {"close"},
				"Content-Length": {"10"},
				HeaderOrderKey:   {"Content-Length", "Connection"},
			},
			Close:         true,
			ContentLength: 10,
		},
	...
	// Chunked response without Content-Length.
	{
		...
		Response{
			...
			ProtoMinor:       1,
			Request:          dummyReq("GET"),
			Header:           Header{HeaderOrderKey: {"Transfer-Encoding"}},
			Close:            false,
			ContentLength:    -1,
	...
	// Trailer header but no TransferEncoding
	{
		...
		Response{
			...
			Header: Header{
				...
				HeaderOrderKey:   {"Trailer", "Content-Length", "Connection"},
			},
			Close:         true,
			ContentLength: 10,
		},
	...
	// Chunked response with Content-Length.
	{
		...
		Response{
			...
			ProtoMinor:       1,
			Request:          dummyReq("GET"),
			Header:           Header{HeaderOrderKey: {"Transfer-Encoding", "Content-Length"}},
			Close:            false,
			ContentLength:    -1,
			TransferEncoding: []string{"chunked"},
		},
	...
	// Content-Length in response to a HEAD request
	{
		...
		Response{
			...
			ProtoMinor:       0,
			Request:          dummyReq("HEAD"),
			Header:           Header{"Content-Length": {"256"}, HeaderOrderKey: {"Content-Length"}},
...
some body`,
		Response{
			...
			Header: Header{
				...
				HeaderOrderKey: {"Connection", "Content-Length"},
			},
			Close:         true,
			ContentLength: -1,
		},
	...
	// 206 Partial Content. golang.org/issue/8923
	{
		...
		Response{
			...
			Header: Header{
				...
				HeaderOrderKey:   {"Content-Type", "Accept-Ranges", "Content-Range", "Content-Length"},
			},
			ContentLength: 6,
		},
	...
	// Both keep-alive and close, on different Connection lines. (Issue 8840)
	{
		...
		Response{
			...
			Header: Header{
				"Content-Length": {"256"},
				HeaderOrderKey:   {"Content-Length", "Connection", "Connection"},
			},
			TransferEncoding: nil,
			Close:            true,
			ContentLength:    256,
	...
	// With a Content-Length.
	{
		...
		Response{
			...
			Header: Header{
				"Content-Length": {"10"},
				HeaderOrderKey:   {"Transfer-Encoding", "Content-Length"},
			},
			Close:         true,
			ContentLength: 10,
		},
...

	{
		...
		Response{
			...
			Header: Header{
				...
				"Connection":       {"keep-alive"},
				"Keep-Alive":       {"timeout=7200"},
				HeaderOrderKey:     {"Content-Encoding", "Content-Length", "Connection", "Keep-Alive"},
			},
			Close:         false,
			ContentLength: 23,
		},
	...
	// Issue 19989: two spaces between HTTP version and status.
	{
		...
		Response{
			...
			Header: Header{
				"Content-Type":     {"text/html"},
				"Www-Authenticate": {`Basic realm=""`},
				HeaderOrderKey:     {"Content-Type", "WWW-Authenticate"},
			},
			Close:         true,
			ContentLength: -1,
		},
