...
import (
	...
	internal "github.com/BRUHItsABunny/oohttp/internal"
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	// This must be checked before DialContext/Dial hooks, because httptest
	// now sets DialContext by default (since go1.26).
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod, isClientConn bool, internalStateHook func()) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		...
		// NOTE: oohttp accepts any TLSConn (e.g. utls) here, not just
		// *tls.Conn; upstream's duck typing already covers that case.
	...
	if s := pconn.tlsState; s != nil && s.NegotiatedProtocolIsMutual && s.NegotiatedProtocol != "" {
		tlsConn, tlsConnOK := pconn.conn.(TLSConn)
...
// decompressResponse transparently decodes resp's body when the response
// advertises a Content-Encoding this Transport knows how to decode.
//
// This is a github.com/BRUHItsABunny/oohttp extension: the stdlib only decodes
// responses to requests where it added "Accept-Encoding: gzip" itself, but
// clients of this fork set their own Accept-Encoding in order to match the
// fingerprint of the browser they impersonate, and still expect a plaintext
// body. The stdlib's opt-outs are still honoured: DisableCompression, HEAD
// requests (no body) and range requests (a partial body cannot be decoded).
func (t *Transport) decompressResponse(req *Request, resp *Response) {
	if t.DisableCompression || req == nil || req.Method == "HEAD" || req.Header.Get("Range") != "" {
		return
	}
	...
	registry := t.DecompressionRegistry
	if registry == nil {
		registry = DefaultDecompressionRegistry
	}
	// Only decode when every encoding in the chain is supported; otherwise
	// hand the body to the caller untouched.
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		pc.t.decompressResponse(rc.treq.Request, resp)
